...
#endif
{
    ...
    if (target_state != current_jtag_state)
    {
        ...
        if (mode_stream_tms) // ITS modification, writes stream of TMS instead of individual bits
        {
            if(count != 0) // If not empty write, send all bits at ones
            {
                jtag_w_TMS_var_mode(count, tms_bits);
            }
        }
        else
        {
    ...
    if(mode_stream_tms) // ITS modification, writes stream of TMS instead of individual bits
    {
        if(cycles != 0) // If not empty write, send all bits at ones
        {
            jtag_w_TMS_var_mode(cycles, 0);
        }
    } else {
