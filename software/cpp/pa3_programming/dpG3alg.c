...
void dp_check_device_ID(void)
{
    ...
    * Therefore, it can be used to store the Data file ID for */
    ...
    //device_ID &= global_ulong1; // ITS modified. Fixes bug. It now uses the device_ID read from the device instead of comparing the one read from the header twice
    device_ID = device_ID & 0x3FFFFFF; // ITS modified. Mask valid bits, verified from BSDL
...
void dp_exit(void)
{
    if(speed_up) // TODO: Add documentation #156
    {
        // 5 + 6 + 8 + 1 + 3 + 5 + 6 + 1 + 3 + 5 = 43
        uint64_t tms_data_64b = 0x7d803340006;
        uint64_t tdi_data_64b = 0x3f0040fe0;
        jtag_64b_sp(tms_data_64b, tdi_data_64b, 43u);
        current_jtag_state = 1;
    }
    else
    {
        opcode = ISC_DISABLE;
    ...
    if (error_code == DPE_SUCCESS)
    {
        if (dat_support_status & CORE_DAT_SUPPORT_BIT)
        {
            ...
            if ((dat_support_status & CORE_DAT_ENCRYPTION_BIT) == 0U)
            {
                ...
                {
                    bol_eol_verify = BOL;
                    //dp_verify_array(); // ITS modification: Verify skipped, check githash instead
                    if (error_code == DPE_SUCCESS)
                    {
                        dp_program_rlock();
                        if (error_code == DPE_SUCCESS)
...
void dp_poll_device(void)
{
    ...
    for ( global_ulong1 = 0U; global_ulong1 <= MAX_PROGRAM_POLL; global_ulong1++ )
    {
        ...
        //DRSCAN_out(5u, (DPUCHAR*)DPNULL, &global_uchar1); // TODO: Document why different than Microsemi standard #156
        goto_jtag_state(JTAG_RUN_TEST_IDLE,0u);
        global_uchar1 = 4; // this is fake read back // TODO: Document why different than Microsemi standard #156
