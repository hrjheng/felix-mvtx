...
feeids =[feeid|0<<12 for feeid in range(12)]
feeids+=[feeid|1<<12 for feeid in range(16)]
feeids+=[feeid|2<<12 for feeid in range(20)]

data=[]
for i,(vcasn,ithr) in enumerate(vcasn_ithr_list):
    for feeid in feeids:
        fname='%s/thrmap%d-%d.dat'%(path,feeid,i)
        if not os.path.isfile(fname): continue
        thrs=np.fromfile(fname,dtype=np.float32)
#        print(fname,thrs,thrs.shape)
        if not len(thrs): continue
        thrs=thrs.reshape((6,9,1024)).swapaxes(0,1)
        for chip in range(9):
            data.append((vcasn,ithr,feeid,chip,np.mean(thrs[chip]),np.var(thrs[chip]),np.sum(thrs[chip]!=0)))
...
fig,axes=plt.subplots(ncols=9,nrows=10,sharex=True,sharey=True)
...
for feeid in feeids:
    ...
    for chip in range(9):
        ...
        axis=axes[stave-6-2*layer][chip]
        axis.grid()
        if (chip==0): axis.set_ylabel("stave %d"%stave)
        if (stave==9): axis.set_xlabel("chip %d"%chip)
