"""MVTX implementation of the BSP.
...
class FlxBsp(Roc):
    """
    MVTX-defined wrapper for BSP
    several functions imported from equivalent class in cru_supported_software subtree
    """
    ...
    def get_short_hash(self):
        ...
        val = self.roc_read(FLXADD['add_bsp_info_shorthash'])
        return val
    ...
    def get_build_date(self):
        ...
        val = self.roc_read(FLXADD['add_bsp_info_builddatetime']) >> 16
        return val

    def get_build_time(self):
        """ get firmware build time """
        val = self.roc_read(FLXADD['add_bsp_info_builddatetime']) & 0xFFFF
        return val

    def get_chip_id(self):
        """ get XCKU chip ID """
        id = self.roc_read(FLXADD['add_bsp_hkeeping_fpgaid'])
        return id

    def log_fw_info(self):
        """ Gets firmware information (hash number, build date and time and FPGA Chip Id """
     #   id=
        self.logger.info('=========================================================================================')
        ...
        self.logger.info('>>> Firmware build date and time={:06X}  {:04X}'.format(self.get_build_date(),self.get_build_time()))
        self.logger.info('>>> FPGA chip ID=0x{:08x}'.format(self.get_chip_id()))
        self.logger.info('=========================================================================================')
