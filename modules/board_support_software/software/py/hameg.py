...
class Hameg:
    ...
    def setup_standalone(self):
        """Setup options when script is run in standalone mode"""
        self.setup_logging()
    ...
    def setup_logging(self):
        ...
        log_file = os.path.join(logdir, "hameg.log")
        log_file_errors = os.path.join(logdir,
                                       "hameg_errors.log")

        fh_std = logging.FileHandler(log_file)
        fh_std.setLevel(logging.DEBUG)

        fh_errors = logging.FileHandler(log_file_errors)
    ...
    def _check_max_channels(self):
        ...
        model_nr = re.search(r'HMP(\d+)', model)
        if not model_nr:
            # JS: right after boot, this seems to return an empty string, so repeat
            model = self.get_model()
        model_nr = re.search(r'HMP(\d+)', model)
        if not model_nr:
        ...
        elif model_nr.group(1) == '4030':
            self.number_channels = 3
        elif model_nr.group(1) == '2030':
            self.number_channels = 3
        elif model_nr.group(1) == '2020':
    ...
    def set_remote_control(self):
        """Turns on remote control"""
        self._write('SYST:REM\n')

    def set_mix_control(self):
        """Enables both remote and local control"""
        self._write('SYST:MIX\n')

    def lock_front_panel(self):
        """Locks the front panel, run set_local_control() to return control"""
    ...
    def get_channel_config_all(self):
        ...
        for channel in range(1, self.number_channels+1):
            ...
            msg = "Channel {0}: {1:.4f}V, {2:.4f}A, {3}".format(
                channel, voltage, current, active_str[active])
            self.logger.info(msg)
            print(msg)

    def fuse_off(self, channel):
        assert channel > 0 & channel <= self.number_channels
        self._write('INST OUT{0}\n'.format(channel))
        self._write('FUSE OFF\n')

    def fuse_on(self, channel):
        assert channel > 0 & channel <= self.number_channels
        self._write('INST OUT{0}\n'.format(channel))
        self._write('FUSE ON\n')
    ...
    def get_power(self, channel):
        ...
        msg = "Channel {0}: {1:.2f} V, {2:.0f} mA, {3:.2f} W".format(
