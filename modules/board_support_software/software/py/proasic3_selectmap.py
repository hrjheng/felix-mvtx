...
class ProAsic3Selmap(SelectMapSpec):
    ...
    def get_write_cnt(self):
        return self.sm_read(Pa3Register.SMAP_WRITE_CNT)

    def get_idcode(self):
        idcode = self.sm_read(XIL_ADDR.IDCODE)
        self.sm_write(Pa3Register.SMAP_CMD, SmapCmdOpcode.WR_FINISHED | EXEC)
        return idcode

    def log_idcode(self):
        self.logger.info(f"Selectmap read of IDcode returns 0x{self.get_idcode():08X}")
    ...
    def read_frames(self, address_list):
        ...
        for i, adr in enumerate(address_list):
            retval = self.configuration_memory_read_procedure(adr)
            data.append(retval)
        return data
    ...
    def clear_and_init_xilinx(self):
        """ Pulls down prog_b line clearing the Xilinx configuration memory. Waits for Xilinx to
            respond by pulling down init_b. When init_b is released, the Xilinx is successfully cleared.
            This is the first command to write when doing an initial configuration."""
        self.write_reg(Pa3Register.SMAP_CMD, SmapCmdOpcode.INIT_XILINX | EXEC)
    ...
    def abort(self):
        """invalidate any transaction currently on the SelectMAP bus, makes the SelectMap bus go into a safe state"""
        self.write_reg(Pa3Register.SMAP_CMD, SmapCmdOpcode.ABORT | EXEC)

    def configuration_memory_read_procedure(self, startaddr=0, nframes=1):
        ...
        for item in (steps):
            self.write4bytes(item)  # WRITE
        for _ in range(123 + 11):
            _junk = self.read4bytes()
        for _ in range(123 * nframes):
