"""
updated from 208 to 209 jan 2020, AV
updated from 207 to 208 may 2019,
207 is in commit b71bb8f492034ad17ec144f0d0ab4f085acd4f7f and earlier
"""
...
class Pa3Register(IntEnum):
    ...
    MAJOR_VERSION = 0x1         # Firmware version
                                #  [7:4] A = alpha, B = Beta, 0 = final release, D = debug release (+UART , -ECC default),
                                #    E = debug release 2 (-UART, -ECC default), F = debug release 3 (+UART, +ECC default)
                                #  [3:0] Major Num
    CLR_CLK_LOL_CNTS = 0x2      # [0] Clear clock lol counters
    STATUS_CLOCK = 0x3          # [0] lcl_clk_lol
                                # [1] lcl_clk_c1b
                                # [2] lcl_clk_c2b
    CNFG_CLOCK = 0x4            # [0] lcl_clk_in_sel
                                # [1] lcl_clk_cs
                                # [2] lcl_clk_rst
    LOCAL_CLK_LOL_CNT = 0x5     # Loss of lock counter lol
    LOCAL_CLK_C1B_CNT = 0x6     # Loss of lock counter c1b
    LOCAL_CLK_C2B_CNT = 0x7     # Loss of lock counter c2b
    ...
    CC_BS_KEY = 0xE             # Key to arm scrubbing. Needs to be 0xA5 to allow scrubbing.
    ...
    BS_START_0 = 0x60           # Page address where scub image starts
    BS_START_1 = 0x61           # Start_0 [7:0], Start_1 [15:8],
    BS_START_2 = 0x62           # Start_2 [18:16]
                                # If value = 0x00000 – Choose default ptr

    BS_STOP_0 = 0x63            # Page address where scub image stops
    BS_STOP_1 = 0x64            # Stop_0 [7:0], Stop_1 [15:8],
    BS_STOP_2 = 0x65            # Stop_2 [18:16]
                                # If value = 0x00000 – Choose default ptr
    ...
    FLASH_MIS_CNT = 0x23        # Debug feature. Number of pattern recognition errors if 'Verify Pattern' is selected. This counter is don't care otherwise.
    FLASH_ROW_ADDR1 = 0x24      # (7:6) - Block address[1:0], (5:0) Page address
    FLASH_ROW_ADDR2 = 0x25      # Block address[9:2]
    FLASH_ROW_ADDR3 = 0x26      # (7:3) x"0", (2:0) Block address[12:10]
    ...
    FLASH_TRX_SIZE_MSB = 0x28   # Page size on Flash [12:8]ECC_FIFO_TMR_ERROR
    ...
    FLASH_ST_WRD = 0x4E         # Status word from Flash after a transaction is finished
                                ...
                                #  [7:1] Don't care
    FIFO_DATA_RD = 0x30         # Return data from ECC FIFO (anything read from flash will be here) NOTE!!! MUST NOT BE READ DURING INIT CONFIG OR SCRUBBING
    FIFO_DATA_WR = 0x31         # Data to write to Flash.
                                #  WHEN WRITING TO FLASH THE NUMBER OF BYTES WRITTEN TO THE FLASH MUST MATCH PAGE SIZE GIVEN BY FLASH_TRX_SIZE_(LSB/MSB)
    ...
    FIFO_WRITER_STATUS = 0x34   # Status register of FIFO write controller
                                ...
                                #  [5] TMR Error in FIFO
    FIFO_WRITER_TMR_ERR = 0x35  # Number of TMR errors in triplicated Write FIFO
    ECC_COMMAND = 0x38          # ECC Command and Control register
    ...
    ECC_ECCSB_CNT = 0x3C        # ECC single bit error counter
    DIPSWITCH1 = 0x40           # Status of connected dipswitches (Default value is depending position of switches) [7:0]
    DIPSWITCH2 = 0x41           # [9:8]
    PUSHBUTTON = 0x42           # [3:0] Pushbutton Values. Always 0 unless pressed down.
                                #   Pushbutton[0] is used as master reset so this will always show up as 0.
    CTRL_LEDS = 0x43            # [1:0] Value to put out on LEDs of PA3
    CRC_0 = 0x49                # [7:0] Calculated CRC-32 value from the CRC checker
    CRC_1 = 0x4A                # [15:8]
    CRC_2 = 0x4B                # [23:16]
    CRC_3 = 0x4C                # [31:24]
    DISABLE_PROGRAMMING = 0x4F  #
    CRC_WRITE_CNT = 0x54        # Number of writes to the CRC module
    HASH_0 = 80                 # [7:0] Shorthash of commit file was generated from
    HASH_1 = 81                 # [15:8]
    HASH_2 = 82                 # [23:16]
    HASH_3 = 83                 # [31:24]
    UPCOUNTER_0 = 85            # [7:0] Time since last power on: T = value * 67.108864s
    UPCOUNTER_1 = 86            # [15:8] Time since last power on
    RST_UPCOUNTER_0 = 87        # [7:0] Time since last global reset: T = value * 67.108864s
    RST_UPCOUNTER_1 = 88        # [15:8] Time since last global reset
...
class CcCmdOpcode(IntEnum):
    ...
    INIT_CONFIG_2 = 0x20 # Init from golden image
...
class RcState(IntEnum):
    ...
    CONFIG_PARAMETER_OK = 4
    BS_PARAMETERS_OK = 5
    PARAMETER_ERROR = 6
    CONFIG2_PARAMETER_OK = 7
