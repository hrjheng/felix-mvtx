...
def get_bad_blocks_of_board(board_id: str, conn, exclusive_name=False, verbose=True):
    ...
    if lookup.empty:
        if verbose:
            print(f"failed to find {board_id}{f' or {othername}' if not exclusive_name else ''}")
        raise LookupError(f"failed to find {board_id}{f' or {othername}' if not exclusive_name else ''}")
...
def store_bad_blocks(id, bad_blocks_d: Dict[int, List[int]], conn):
    ...
    bad_blocks_dict_check(bad_blocks_d)
    ...
    with conn:
        table.to_sql('bad_blocks', conn, if_exists='append', index_label='id', index=False)
    # lookup2 = pd.read_sql_query('SELECT * from bad_blocks', conn)
    return table


def bad_blocks_dict_check(bad_blocks_d):
...
if __name__ == '__main__':
    ...
    # print(*map(hex, good_blocks))
    con2 = sqlite3.connect(':memory:')
    # store_bad_blocks("non", {0: [0, 9, 2, 3, 4], 1: [5, 9, 2, 3, 4]}, con2)
    store_bad_blocks("non", {0: [5, 9, 10, 0x100], 1: [5, 9, 2, 3, 4]}, con2)
    store_bad_blocks("non", {0: [5, 9, 2, 0x99], 1: [5, 9, 2, 3, 4]}, con2)
    ...
    already_exists("non", con2)

    test_dict = {
        0:['0x3ea', '0x3ea', '0xec8', '0xec8', '0x11d7', '0x11d7', '0x12fa', '0x12fa', '0x1567', '0x1567', '0x1f68', '0x1f68'],
        1:['0x436', '0x436', '0x1288', '0x1288', '0x12d7', '0x12d7', '0x15cc', '0x15cc', '0x162a', '0x162a', '0x1805', '0x1805', '0x185c', '0x185c', '0x1b86', '0x1b86', '0x1f6e', '0x1f6e']
    }
    test_dict = {k:[int(x,16) for x in v] for k,v in test_dict.items()}
    store_bad_blocks("2038", test_dict, con2)
    store_bad_blocks("2038", test_dict, con2)
