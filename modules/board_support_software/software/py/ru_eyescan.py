...
from ru_gthe3_channel_drp_mapping import Gthe3ChannelAddress as gthadd
from ru_gthe3_channel_drp_mapping import Gthe3ChannelLow as gthadd_bitlow
from ru_gthe3_channel_drp_mapping import Gthe3ChannelWidth as gthadd_bitwidth


class EyeScan(object):
    """Encapsulates Eye scan functionality"""

    def __init__(self, transceiver, number=None):
    ...
    def initialize(self, is_lpm_equalizer=True, horizontal_max=128, vertical_max=127):
        ...
        self._set_sdata_mask(bus_width=20)
        self._set_qual_mask()
    ...
    def _eye_scan_point_ut(self, offset_h, offset_v, prescale, ut_sign, func, readback):
        """Perform an eye scan at given offset points (horizontal, vertical, ut_sign), with given prescale"""

        self._eye_scan_point_setup(offset_h, offset_v, prescale, ut_sign, readback=readback)
    ...
    def _eye_scan_point(self, offset_h, offset_v, prescale, ber, func, readback):
        ...
        cnt = [0, 0]
        while not done:
            cnt1 = self._eye_scan_point_ut(offset_h, offset_v, cps, 0, func, readback)
            if self.is_lpm:
                cnt2 = (0, 0)
            else:  # DFE
                cnt2 = self._eye_scan_point_ut(offset_h, offset_v, cps, 1, func, readback)
            cnt[0] += cnt1[0] + cnt2[0]
            cnt[1] += cnt1[1] + cnt2[1]
            ...
            for j in range_j:
                ...
                if (point_x, point_y) not in table:
                    if zeroCnt == 0 and point_y < zeroPt and no_center:
                        ...
                        start = time.time()
                        cnt = self._eye_scan_point(point_x, point_y, prescale, ber, func, readback)
    ...
    def eye_scan_adaptive(
        ...
        """
        assert isinstance(ber,int)
        assert isinstance(prescale,int)
        assert isinstance(threshold,int)
        assert ber >= prescale
        assert threshold in range(1,2**16)
        ...
        if resume and os.path.isfile(output_file_name):
            ...
            if resume:
                self.logger.warning(f"{output_file_name} not existing, creating")
            ...
            for point_x in pointsToRun.keys():
                ...
                for point_y in pointsToRun[point_x]:
                    ...
                    if no_center and point_y > 0 and zerosample_pos > 0:
                        ...
                        cnt = self._eye_scan_point(point_x, point_y, ps, ps, func, readback=readback_config)
                        if cnt[1] == 0:
                            if point_y > 0:
...
class EyeScanGtx(EyeScan):
    ...
    def __init__(self, transceiver):
        super(EyeScanGtx, self).__init__(transceiver)
...
class EyeScanGth(EyeScan):
    """Eye scan functionality for GTH transceiver"""
    # DRP addresses
    ES_CONTROL = gthadd.ES_CONTROL.value         # 0x3C
    ES_EYE_SCAN_EN = gthadd.ES_EYE_SCAN_EN.value # 0x3C
    ES_PRESCALE = gthadd.ES_PRESCALE.value       # 0x3C
    ES_QUAL_MASK = (gthadd.ES_QUAL_MASK4.value,
                    gthadd.ES_QUAL_MASK3.value,
                    gthadd.ES_QUAL_MASK2.value,
                    gthadd.ES_QUAL_MASK1.value,
                    gthadd.ES_QUAL_MASK0.value) # (0x48,0x47,0x46,0x45,0x44) 4:0
    ES_SDATA_MASK = (gthadd.ES_SDATA_MASK4.value,
                     gthadd.ES_SDATA_MASK3.value,
                     gthadd.ES_SDATA_MASK2.value,
                     gthadd.ES_SDATA_MASK1.value,
                     gthadd.ES_SDATA_MASK0.value) # (0x4D, 0x4C, 0x4B, 0x4A, 0x49) 4:0
    ES_HORZ_OFFSET = gthadd.ES_HORZ_OFFSET.value # 0x4F
    ES_VERT_OFFSET = 0x97 # Not existing on GTH: equivalence from UG576 v1.6 Table 4-20 P232
    ES_CONTROL_STATE = gthadd.es_control_status.value # 0x153
    ES_SAMPLE_COUNT = gthadd.es_sample_count.value    # 0x152
    ES_ERROR_COUNT = gthadd.es_error_count.value      # 0x151
    ...
    def __init__(self,transceiver,vertical_range=0):
        assert len(transceiver.transceivers) == 1, "GTH frontend with multiple transceivers not supported"
        super(EyeScanGth, self).__init__(transceiver, number=transceiver.transceivers[0])

        vertical_range = VerticalRange(vertical_range)

        self.eyescan_vs_range = vertical_range.value
    ...
    def get_vertical_range(self):
        ...
        return float(VerticalRange(self.eyescan_vs_range).name[2:5].replace('_','.'))

    def initialize(self):
        super(EyeScanGth, self).initialize()

        # stores vertical offsets defaults bits [12:11]
        # i.e. RX_AFE_CM_EN, RX_CAPFF_SARC_ENB
        self.vert_offset_defaults = self.transceiver.read_drp(self.ES_VERT_OFFSET) & (1<<11 | 1<<12)
        # stores horizontal offset default bits [3:0]
        # i.e. FTS_LANE_DESKEW_CFG
        self.horz_offset_defaults = self.transceiver.read_drp(self.ES_HORZ_OFFSET) & 0x000F

    def _reset_receiver_pma(self):
        """Always after enable: UG576 v1.6 Table 4-20 P230"""
        self.transceiver.reset_receivers()

    def _activate_eye_scan(self):
        """UG576 v1.6 P378: enable eye and error"""
        value = 0
        value |= 0 << gthadd_bitlow.ES_CONTROL.value
        value |= 1 << gthadd_bitlow.ES_ERRDET_EN.value
        value |= 1 << gthadd_bitlow.ES_EYE_SCAN_EN.value
        value |= 0 << gthadd_bitlow.ES_PRESCALE.value
        self.transceiver.write_drp(self.ES_EYE_SCAN_EN, value)

    def _set_qual_mask(self):
        """Set qualifier mask for Eye scan: UG576 v1.6 Table 4-20"""
        for i in self.ES_QUAL_MASK: # set all to 1 as in P224
            self.transceiver.write_drp(i, 0xFFFF)
    ...
    def _set_sdata_mask(self, bus_width):
        """Set the sdata mask: see UG576 v1.6 Table 4-20"""
        # assume 40 bit mask
        es_sdata_mask = [0xFFFF, 0xFFFF, 0xFF00, 0x0000, 0x0000]
        # NOTE: es_sdata_mask[0] goes to ES_SDATA_MASK4 for better visualisation
        # NOTE: es_sdata_mask[4] goes to ES_SDATA_MASK0 for better visualisation
        if bus_width == 40:
            pass
        elif bus_width == 32:
            es_sdata_mask[3] = 0x0000
            es_sdata_mask[4] = 0x00FF
        elif bus_width == 20:
            es_sdata_mask[3] = 0x000F
            es_sdata_mask[4] = 0xFFFF
        elif bus_width == 16:
            es_sdata_mask[3] = 0x00FF
            es_sdata_mask[4] = 0xFFFF
        else:
            raise ValueError('Only 40,32,20,16 are allowed')
        for mask_addr, mask_data in zip(self.ES_SDATA_MASK, es_sdata_mask):
            self.transceiver.write_drp(mask_addr, mask_data)
    ...
    def _set_offset_h(self, offset_h):
        """set horizontal offset for an eye scan"""
        self.offset_h = offset_h
        data = ((offset_h & (2**(gthadd_bitwidth.ES_HORZ_OFFSET.value)-1))<<gthadd_bitlow.ES_HORZ_OFFSET.value) | self.horz_offset_defaults
        self.transceiver.write_drp(self.ES_HORZ_OFFSET, data)

    def _get_offset_h(self):
        """get horizontal offset for an eye scan"""
        offset_h_reg = self.transceiver.read_drp(self.ES_HORZ_OFFSET)
        return (offset_h_reg>>gthadd_bitlow.ES_HORZ_OFFSET.value) & 2**(gthadd_bitwidth.ES_HORZ_OFFSET.value)-1
    ...
    def _check_eye_scan_settings(self, offset_h, offset_v, prescale, ut_sign):
        ...
        offset_h_rb = self._get_offset_h()
